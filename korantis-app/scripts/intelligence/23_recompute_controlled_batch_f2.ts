...
function decision(outputs: RepairedOutput[]): string {
  ...
  if (visionEvaluated.length / outputs.length < 0.95 || photoHeroRate < 0.85) return 'C) Improve photo intelligence';
  if (underEvidencedWork >= Math.ceil(cafes.length * 0.5)) return 'B) Improve practical evidence sources';
  if (!activeRejectedCredible) return 'D) Adjust eligibility scoring';
...
function report(outputs: RepairedOutput[], originalOutputs: OriginalOutputFile): string {
  ...
  const newPhotoRejected = outputs.filter((output) => output.previous_eligibility_status !== 'rejected' && output.eligibility.status === 'rejected' && output.eligibility.reasons.some((reason) => reason.includes('photo') || reason.includes('hero')));
  ...
  const practicalRecordCount = outputs.reduce((sum, output) => sum + output.diagnostics.practical_evidence_items, 0);
  const repairedVisionPass = repairedMatches.filter((output) => output.photo_intelligence.evaluation_status === 'evaluated' && output.photo_intelligence.acceptable_hero_photo);

  return [
    '# Controlled Batch 30 F.2 Intelligence Report',
    '',
    ...
    `- Practical evidence import records used: ${practicalRecordCount}`,
    `- Decision: ${finalDecision}`,
    '',
    '## Required Answers',
    '',
    `1. Did the 5 repaired venues pass photo vision? ${repairedVisionPass.length === 5 ? 'Yes' : `No: ${repairedVisionPass.length}/5 passed.`}`,
    `2. Did any pending_review become active? ${pendingBecameActive.length ? `Yes: ${pendingBecameActive.map((output) => output.venue_name).join(', ')}` : 'No'}.`,
    `3. Did any venue become rejected due to poor photo quality? ${newPhotoRejected.length ? `Yes: ${newPhotoRejected.map((output) => output.venue_name).join(', ')}` : 'No new venue became rejected due to photo quality.'}`,
    `4. Are cafes still under-evidenced for work? ${workUnderEvidenced.length ? `Yes: ${workUnderEvidenced.length}/${cafes.length} cafes still have weak work/practical evidence.` : 'No'}.`,
    `5. Is La Malbequeria still rejected? ${malbequeria?.eligibility.status === 'rejected' ? `Yes: ${malbequeria.eligibility.reasons.join('; ') || 'rejected'}.` : 'No'}`,
    ...
    '- Repaired matches now have photo vision coverage from F.2.',
...
function main() {
  ...
  const repairedVision = readJson<VisionFile>(path.join(process.cwd(), 'data', 'controlled_batch_30_repaired_vision_results.json'));
  ...
  writeFileSync(path.join(process.cwd(), 'data', 'controlled_batch_30_f2_intelligence.json'), JSON.stringify({
  ...
  writeFileSync(path.join(process.cwd(), 'data', 'controlled_batch_30_f2_intelligence_report.md'), markdown);

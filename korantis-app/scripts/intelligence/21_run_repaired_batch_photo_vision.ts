...
function unevaluatedVenue(record: BatchGoogleRecord, reason: string): VenueVisionResult {
  return {
    candidate_id: record.candidate_id,
...
function report(venues: VenueVisionResult[], dryRun: boolean): string {
  ...
  return [
    '# Controlled Batch 30 Repaired Vision Report',
    '',
    `Generated: ${new Date().toISOString()}`,
    '',
    `- Dry run: ${dryRun ? 'yes' : 'no'}`,
    `- Target repaired venues: ${venues.length}`,
...
async function main() {
  ...
  const enrichment = readJson<EnrichmentFile>(path.join(process.cwd(), 'data', 'controlled_batch_30_google_enrichment_repaired.json'));
  const targetRecords = enrichment.records.filter((record) => TARGET_NAMES.has(record.candidate_name) && record.status === 'matched');
  ...
  console.warn(dryRunReason || `Running repaired photo vision for ${targetRecords.length} venues only.`);

  for (const record of targetRecords) {
    if (!photoInputs(record).length) {
      venues.push(unevaluatedVenue(record, 'no photo references'));
      continue;
    }
    if (!openai || !googleApiKey || dryRunReason) {
      venues.push(unevaluatedVenue(record, dryRunReason || 'photo vision dry-run'));
      continue;
    }
  ...
  writeFileSync(path.join(process.cwd(), 'data', 'controlled_batch_30_repaired_vision_results.json'), JSON.stringify({
    generatedAt: new Date().toISOString(),
    dryRun: Boolean(dryRunReason),
    model: dryRunReason ? null : OPENAI_VISION_MODEL,
    venues,
  }, null, 2));
  writeFileSync(path.join(process.cwd(), 'data', 'controlled_batch_30_repaired_vision_report.md'), markdown);

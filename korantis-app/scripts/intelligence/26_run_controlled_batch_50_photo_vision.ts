...
function report(venues: VenueVisionResult[], dryRun: boolean): string {
  ...
  return [
    '# Controlled Batch 50 Photo Vision Report',
    '',
...
async function main() {
  ...
  if (enrichment.records.length > MAX_CANDIDATES) throw new Error(`Refusing to process more than ${MAX_CANDIDATES} records.`);
  ...
  console.warn(dryRunReason || `Running photo vision for ${matched.length} matched controlled batch 50 venues only.`);
  ...
  writeFileSync(path.join(process.cwd(), 'data', 'controlled_batch_50_photo_vision.json'), JSON.stringify({
    generatedAt: new Date().toISOString(),
    dryRun: Boolean(dryRunReason),
    model: dryRunReason ? null : OPENAI_VISION_MODEL,
    venues,
  }, null, 2));
  writeFileSync(path.join(process.cwd(), 'data', 'controlled_batch_50_photo_vision_report.md'), markdown);
